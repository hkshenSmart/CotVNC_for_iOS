...
- (void)doSetServerPassword:(char *)password {
...
- (void)resetReader
{
    ...
    //    connection.rfbMajorVersion = 3;
    //    connection.rfbMinorVersion = 4;
    int protocolMinorVersion = 4;
    ...
    memset(m_clearpasswd, 0x00, 256);
    if (serverPassword) {
        sprintf(m_clearpasswd, serverPassword);
    } else {
        sprintf(m_clearpasswd, "1011");
    }
    //sprintf(m_clearpasswd, "1011");
    //m_clearpasswd[4] = '\0';
    [target writeBytes:(unsigned char *)m_clearpasswd length:16];
