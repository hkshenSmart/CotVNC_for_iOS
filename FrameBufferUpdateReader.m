...
- (id)initTarget:(id)aTarget action:(SEL)anAction
{
    if (self = [super initTarget:aTarget action:anAction]) {
		...
		headerReader = [[ByteBlockReader alloc] initTarget:self action:@selector(setHeader:) size:sz_rfbFramebufferUpdateMsg - 1];
        ...
        xcursorEncodingReader = [[XcursorEncodingReader alloc] initTarget:self action:@selector(rectsAndHeaderChange:)];
...
- (void)setFrameBuffer:(id)aBuffer
{
    ...
    [xcursorEncodingReader setFrameBuffer:aBuffer];
}
...
- (void)setRect:(NSData*)rectInfo
{
    ...
    NSLog(@"Here e:%d", ntohl(msg->encoding));
    //RFB_ENCODING_POINTER_POS
    if (ntohl(msg->encoding) == -232) {
        numberOfRects--;
        if(numberOfRects) {
            [target setReader:rectHeaderReader];
        } else {
...
//			NSLog(@"ZRLE Encoding");
        ...
        case rfbEncodingXCursor:
            NSLog(@"rfbEncodingXCursor");
            break;
        case rfbEncodingRichCursor:
            NSLog(@"rfbEncodingRichCursor");
            theReader = xcursorEncodingReader;
            break;
        case RFB_ENCODING_POINTER_POS:
            NSLog(@"RFB_ENCODING_POINTER_POS");
            break;
        case rfbEncodingLastRect:
            NSLog(@"rfbEncodingLastRect");
            break;
    }
    ...
    if(theReader == nil) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:STARTVNCAGAIN object:nil];
    } else {
...
- (void)rectsAndHeaderChange:(EncodingReader*)aReader {
    numberOfRects--;
    if(numberOfRects) {
        [target setReader:rectHeaderReader];
    } else {
